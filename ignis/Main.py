...
def main(argv):
	...
	processor.registerProcessor("files", IFilesModuleRpc.Processor(IFilesModuleRpc.Client(files)))
	keys = IKeysModule(executorData)
	processor.registerProcessor("keys", IKeysModuleRpc.Processor(IKeysModuleRpc.Client(keys)))
	mapper = IMapperModule(executorData)
	processor.registerProcessor("mapper", IMapperModuleRpc.Processor(IMapperModuleRpc.Client(mapper)))
	postman = IPostmanModule(executorData)
	processor.registerProcessor("postman", IPostmanModuleRpc.Processor(IPostmanModuleRpc.Client(postman)))
	reducer = IReducerModule(executorData)
	processor.registerProcessor("reducer", IReducerModuleRpc.Processor(IReducerModuleRpc.Client(reducer)))
	server = IServerModule(executorData)
	processor.registerProcessor("server", IServerModuleRpc.Processor(IServerModuleRpc.Client(server)))
	shuffle = IShuffleModule(executorData)
	processor.registerProcessor("shuffle", IShuffleModuleRpc.Processor(IShuffleModuleRpc.Client(shuffle)))
	sort = ISortModule(executorData)
	processor.registerProcessor("sort", ISortModuleRpc.Processor(ISortModuleRpc.Client(sort)))
	storage = IStorageModule(executorData)
	processor.registerProcessor("storage", IStorageModuleRpc.Processor(IStorageModuleRpc.Client(storage)))
